...
<template>
  ...
  <div id="window-container" v-if="showPopup">
    ...
    <div class="form">
      ...
      <form v-if="!isLoginForm" class="register-form">
        ...
        <label>帳號</label>
        <input
          v-model="owner_account"
          type="text"
          placeholder="3-10位英數混合帳號"
        />
        <label>真實姓名</label>
      ...
      <form v-else class="login-form">
        ...
        <p class="message">
          尚未加入會員? <a href="#" @click.prevent="toggleForm">註冊</a>
        </p>
      </form>
    </div>
  </div>
  ...
  <div v-if="!this.loggedIn" class="loginregister">
    <button class="login" @click="customizeWindowEvent" key="login">
      登入/註冊
    </button>
  </div>
  <!-- 登入後顯示的按鈕區域 -->
  <div v-else class="loggedin-buttons" key="button">
    <p>
      親愛的房東&nbsp&nbsp{{ loginObj.ownerAccount }}&nbsp&nbsp&nbsp 已登入~
    </p>
