...
export default {
  ...
  methods: {
    ...
    generatePDF() {
      ...
      doc.text(`（二）持分面積：${this.oneContractObj.acreage}`, 20, 55);
      ...
      doc.text(`（二）水費：每月${this.oneContractObj.waterP}元整。`, 20, 150);
      doc.text(
      ...
      doc.setFontSize(12);
      doc.setFont("NotoSerifTC", "bold");
      doc.text(`六、提前終止租約 `, 10, 165);

      doc.setFontSize(10);
      doc.setFont("NotoSerifTC", "normal");
      doc.text(`如有提前終止租約情事，依下列約定辦理：`, 20, 170);
      doc.text(
        `（一）本契約於期限屆滿前，除不可避免之情形得提前終止租約外，租賃雙方不得任意終止租約。 `,
        20,
        175
      );
      doc.text(`（二）違約金：${this.oneContractObj.cutP}元整。`, 20, 180);

      doc.setFontSize(12);
      doc.setFont("NotoSerifTC", "bold");
      doc.text(`立契約書人 `, 10, 190);

      doc.setFontSize(10);
      doc.setFont("NotoSerifTC", "normal");
      doc.text(`出租人 `, 10, 200);
      doc.text(`姓名：${this.loginObj.ownerName} `, 10, 205);
      doc.text(
        `統一編號（身分證字號）：${this.loginObj.ownerIdentity} `,
        10,
        210
      );
      doc.text(
        `戶籍地址(營業登記地址)： ${this.loginObj.ownerHomeAddress} `,
        10,
        215
      );
      doc.text(`通訊地址： ${this.loginObj.ownerContactAddress} `, 10, 220);

      doc.text(`承租人 `, 10, 230);
      doc.text(`姓名：${this.oneContractObj.tenantName} `, 10, 235);
      doc.text(
      ...
      doc.text(`連絡電話： ${this.oneContractObj.tenantPhone} `, 10, 255);

      doc.setFontSize(12);
      doc.setFont("NotoSerifTC", "bold");
      doc.text(`立約日期： ${formattedDate}`, 10, 270);
...
<template>
  <div class="bigArea">
    ...
    <div class="roomInfo">
      ...
      <div class="rent_time">
        <label for="start_time"
          >租賃期間 自：{{ oneContractObj.startDate }}</label
        >
    ...
    <br />
    <h3 class="signdate">
      立約日期：{{ formatDate(oneContractObj.signDate) }}
    </h3>

    契約編號:{{ oneContractObj.ai }}

    <button @click="this.generatePDF()">列印</button>
    <!-- <div class="btn"> 
